...
export class ArchiveMainPage {
  ...
  search(initial = false) {
    ...
    result = result.filter(m => {
      const bySeason = m.season === this.dataContext.filter.season || this.dataContext.filter.season === this.ALL_SEASONS;
  ...
  hasTextSpeakers(speakers: Speaker[], pattern: string): boolean {
    return !pattern || speakers.map(s => s.names).some(n => this.hasPattern(n, pattern));
  }

  hasPattern(texts: any, pattern: string): boolean {
    return !pattern || JSON.stringify(texts || {}).toLowerCase().indexOf(pattern.toLowerCase()) > -1;
  }
}
